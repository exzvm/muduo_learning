...
// 退出事件循环  1.loop在自己的线程中调用quit    2.非loop线程中，调用loop的quit
/**
 *            mainLoop
 * 
 *                                             no  ====================  生产者-消费者的线程安全的队列
...
void EventLoop::quit() {
  ...
  // 如果在其它线程中调用quit，在一个subloop(worker)中，调用了mainloop(IO)的quit
  if (!isInLoopThread()) { 
    wakeup();
  }
}
...
// 唤醒loop所在线程 向wakeupFd_写一个数据，wakeupChannel就发生读事件，当前loop线程就会被唤醒
